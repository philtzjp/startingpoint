...
- `.agents/skills/skill-setup` による `philtzjp/skills` からの必要スキル導入手順
...
- `skill-setup` を使って、`refresh-skills` と `skill-escalation` を必ず導入すること
- 必要なスキルだけを `philtzjp/skills` から追加導入すること
- 導入後に `skill-setup` 自体を `.agents/skills` と `.claude/skills` から削除すること
...
```text
.
├── .agents/skills/skill-setup/SKILL.md
